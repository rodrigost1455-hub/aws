...
(async function () {
  ...
  const client = id ? await T.getClient(id) : null;
  ...
  const lastVals = await T.lastValues(client.id); // {balances, zillow, privateReserve, quarter} | null
  ...
  async function generate() {
    ...
    let report;
    try {
      report = await T.createReport(client.id, {
        quarter: RQ.label,
        date: RQ.date,
        balances,
        liabilities,
      ...
      if (e.status === 422 && e.body && e.body.missing) {
        AW.toast("Server rejected: " + e.body.missing.length + " missing field(s).");
      } else {
        AW.toast("Generate failed: " + (e.message || "network error"));
      }
      return;
    }
    generated = report;
  ...
  async function renderResult() {
    ...
    const r = generated;
    const c = r.calc;
    const history = await T.listReports(client.id);

    const dlBtn = (cls, icon, title, desc, btnLabel, btnClass, href, action) => {
      const inner = `${AW.icon(cls === "canva" ? "copy" : "download", 14)} ${btnLabel}`;
      const ctrl = href
        ? `<a class="btn ${btnClass}" href="${href}" target="_blank" rel="noopener">${inner}</a>`
        : `<button class="btn ${btnClass}" data-action="${action}">${inner}</button>`;
      return `<div class="dlcard ${cls}">
    ...
    const dl = (cls, icon, title, desc, btnLabel, btnClass) => {
      if (cls === "sacs") return dlBtn(cls, icon, title, desc, btnLabel, btnClass, T.pdfUrl(r.id, "sacs"));
      if (cls === "tcc")  return dlBtn(cls, icon, title, desc, btnLabel, btnClass, T.pdfUrl(r.id, "tcc"));
      return dlBtn(cls, icon, title, desc, btnLabel, btnClass, null, "canva");
    };
              ...
              <tr>
                ...
                <td>
                  <div class="row gap-sm" style="justify-content:flex-end">
                    <a class="btn btn-ghost btn-xs" href="${T.pdfUrl(h.id, "sacs")}" target="_blank" rel="noopener">${AW.icon("download", 12)} SACS</a>
                    <a class="btn btn-ghost btn-xs" href="${T.pdfUrl(h.id, "tcc")}" target="_blank" rel="noopener">${AW.icon("download", 12)} TCC</a>
                  </div>
                </td>
              </tr>`).join("")}
          </tbody>
    ...
    // Canva export → stub returns 501; surface via toast.
    root.querySelectorAll('[data-action="canva"]').forEach((b) =>
      b.addEventListener("click", async () => {
        try {
          await T.exportCanva(r.id);
          AW.toast("Canva export queued.", "ok");
        } catch (e) {
          AW.toast(e.message || "Canva export not configured");
        }
      }));
  }
